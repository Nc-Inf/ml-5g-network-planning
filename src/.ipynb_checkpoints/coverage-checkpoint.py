...
def coverage_score(
    ...
    """
    d = np.linalg.norm(grid_xy[:, None, :] - bs_xy[None, :, :], axis=2)  # (Ngrid, Nbs)
    pr = received_power_dbm(tx_dbm, d, n=2.2)
    best = pr.max(axis=1)
    return float((best > thr_dbm).mean())

def best_power_map(grid_xy: np.ndarray, bs_xy: np.ndarray, tx_dbm: float) -> np.ndarray:
